...
public class WebApplicationExceptionMapper implements ExceptionMapper<Exception> {
	...
	public Response toResponse(Exception exception) {
		...
		return Response.status(Response.Status.INTERNAL_SERVER_ERROR).type(MediaType.APPLICATION_JSON).entity(new Error(exception.getClass().getSimpleName() + ": " + exception.getMessage())).build();
	}
